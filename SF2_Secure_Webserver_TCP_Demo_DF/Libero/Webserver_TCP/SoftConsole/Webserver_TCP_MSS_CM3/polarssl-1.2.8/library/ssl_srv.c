...
static int ssl_write_server_hello( ssl_context *ssl )
{
    ...
    *p++ = (unsigned char) ssl->minor_ver;
    ...
    SSL_DEBUG_MSG( 3, ( "server hello, chosen version: [%d:%d]",
     ...
     MSS_RTC_get_calendar_count(&t);//time( NULL );
#if 0
    ...
    *p++ = (unsigned char)( t       );
#endif
    *p++ = (unsigned char)( t.day);
   *p++ = (unsigned char)(t.hour );
   *p++ = (unsigned char)( t.minute);
   *p++ = (unsigned char)( t.second);
    ...
    {
    	printf("ssl->f_rng( ssl->p_rng, p, 28 ) = %d\t",ret);
    	return( ret );
    }
...
#else
    for( i = 28; i > 0; i-- )
             *p++ = (unsigned char) ssl->f_rng( ssl->p_rng,p,28 );
#endif
    ...
    *p++ = (unsigned char) ssl->session_negotiate->length;
        ...
        ssl->f_get_cache( ssl->p_get_cache, ssl->session_negotiate ) != 0 )
    {
        ...
        if( ( ret = ssl->f_rng( ssl->p_rng, ssl->session_negotiate->id,
                                n ) ) != 0 )
        {
        	printf("ssl->f_rng( ssl->p_rng = %d\t",ret);
        	return( ret );
        }
#else
        for( i = 0; i < n; i++ )
			ssl->session_negotiate->id[i] = (unsigned char) ssl->f_rng( ssl->p_rng,p, 28 );
#endif
    }
    else
    {
    ...
    *p++ = (unsigned char)( ssl->session_negotiate->compression      );
    ...
    {
    	//printf("ssl->secure_renegotiation");
...
static int ssl_write_server_key_exchange( ssl_context *ssl )
{
    ...
    if( ( ret = ssl_write_record( ssl ) ) != 0 )
    {
        ...
        //printf("ssl_write_record1 %d\t",ret);
        return( ret );
    }
    ...
    else
    {
            ...
            ssl->handshake->premaster[1] != ssl->max_minor_ver )
        {
            ...
            for( i = 0; i < ssl->handshake->pmslen ; i++ )
                       	ssl->handshake->premaster[i] = (unsigned char) ssl->f_rng( ssl->p_rng,NULL,ssl->handshake->pmslen );
#endif
        }
    }
...
int ssl_handshake_server_step( ssl_context *ssl )
{
    ...
    switch( ssl->state )
    {
        ...
        case SSL_CLIENT_HELLO:
            ...
            //printf("\tSSL_CLIENT_HELLO 2 = %d\t",ret);
            break;
        ...
        case SSL_SERVER_HELLO:
            ...
            //printf("\tSSL_SERVER_HELLO 3 = %d\t",ret);
            break;

        case SSL_SERVER_CERTIFICATE:
            ret = ssl_write_certificate( ssl );
            //printf("\tSSL_SERVER_CERTIFICATE 4 = %d\t",ret);
            break;

        case SSL_SERVER_KEY_EXCHANGE:
            ret = ssl_write_server_key_exchange( ssl );
            //printf("\tSSL_SERVER_KEY_EXCHANGE 5 = %d\t",ret);
            break;

        case SSL_CERTIFICATE_REQUEST:
            ret = ssl_write_certificate_request( ssl );
            //printf("\tSSL_CERTIFICATE_REQUEST 6 = %d\t",ret);
            break;

        case SSL_SERVER_HELLO_DONE:
            ret = ssl_write_server_hello_done( ssl );
            //printf("\tSSL_SERVER_HELLO_DONE 7 = %d\t",ret);
            break;
        ...
        case SSL_CLIENT_CERTIFICATE:
            ...
            //printf("SSL_CLIENT_CERTIFICATE 8 = %d\t",ret);
            break;
#if 1
        case SSL_CLIENT_KEY_EXCHANGE:
            ret = ssl_parse_client_key_exchange( ssl );
            //printf("SSL_CLIENT_KEY_EXCHANGE 9 = %d\t",ret);
            break;

        case SSL_CERTIFICATE_VERIFY:
            ret = ssl_parse_certificate_verify( ssl );
            //printf("SSL_CERTIFICATE_VERIFY 10 = %d\t",ret);
            break;

        case SSL_CLIENT_CHANGE_CIPHER_SPEC:
            ret = ssl_parse_change_cipher_spec( ssl );
            //printf("SSL_CLIENT_CHANGE_CIPHER_SPEC 11 = %d\t",ret);
            break;

        case SSL_CLIENT_FINISHED:
            ret = ssl_parse_finished( ssl );
            //printf("SSL_CLIENT_FINISHED 12 = %d\t",ret);
            break;
