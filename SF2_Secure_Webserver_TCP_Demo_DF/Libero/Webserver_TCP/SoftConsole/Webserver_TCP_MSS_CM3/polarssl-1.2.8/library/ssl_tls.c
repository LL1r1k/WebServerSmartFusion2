    ...
    else
    {
        ...
        if( ssl->minor_ver >= SSL_MINOR_VERSION_2 )
        {
            ...
            if( ret != 0 )
                return( ret );
#else
            for( i = 0; i < ssl->transform_out->ivlen; i++ )
                   		 ssl->transform_out->iv_enc[i] = (unsigned char) ssl->f_rng( ssl->p_rng,NULL,ssl->transform_out->ivlen );
#endif
            /*
             * Shift message for ivlen bytes and prepend IV
             */
...
int ssl_read_record( ssl_context *ssl )
{
        ...
        ssl->in_msgtype != SSL_MSG_APPLICATION_DATA )
    {
        ...
        if( ( ret = ssl_send_alert_message( ssl,SSL_ALERT_LEVEL_FATAL,SSL_ALERT_MSG_UNEXPECTED_MESSAGE ) ) != 0 )
        {
            return( ret );
        }
...
int ssl_parse_certificate( ssl_context *ssl )
{
    ...
    while( i < ssl->in_hslen )
    {
        ...
        ret = x509parse_crt_der( ssl->session_negotiate->peer_cert, ssl->in_msg + i, n );
        if( ret != 0 )
        {
