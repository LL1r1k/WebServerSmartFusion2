	 ...
	 switch( keysize )
	  {
	    case 128:
	      ctx->nr = 10;
	      memcpy(ctx->enc_key, key, 16);
	      break;
	    case 192:
	      ctx->nr = 12;
	      memcpy(ctx->enc_key, key, 24);
	      break;
	    case 256:
	      ctx->nr = 14;
...
int aes_setkey_dec( aes_context *ctx, const unsigned char *key, unsigned int keysize )
{
  ...
  switch( keysize )
  {
    case 128:
      ctx->nr = 10;
      memcpy(ctx->dec_key, key, 16);
      break;
    case 192:
      ctx->nr = 12;
      memcpy(ctx->dec_key, key, 24);
      break;
    case 256:
      ctx->nr = 14;
                    ...
                    unsigned char *output )
{
    ...
    if( mode == AES_DECRYPT )
    {
    	...
    	MSS_SYS_256bit_aes(ctx->dec_key,iv,length/16,MSS_SYS_CBC_DECRYPT,output,input);
#else
        while( length > 0 )
        {
    ...
    else
    {
    	...
    	MSS_SYS_256bit_aes(ctx->enc_key,iv,length/16,MSS_SYS_CBC_ENCRYPT,output,input);
#else
